...
namespace Lib\PhpAmqpLib\Connection;

use Lib\PhpAmqpLib\Wire\IO\SocketIO;
...
class AMQPSocketConnection extends AbstractConnection {
    ...
    $host, $port, $user, $password, $vhost = '/', $insist = false, $login_method = 'AMQPLAIN', $login_response = null, $locale = 'en_US', $read_timeout = 3, $keepalive = false, $write_timeout = 3, $heartbeat = 0
    ) {
        //SocketIO实例
    ...
    protected static function try_create_connection($host, $port, $user, $password, $vhost, $options) {
        ...
        $read_timeout = isset($options['read_timeout']) ? $options['read_timeout'] : 3;
        $keepalive = isset($options['keepalive']) ? $options['keepalive'] : false;
        $write_timeout = isset($options['write_timeout']) ? $options['write_timeout'] : 3;
        $heartbeat = isset($options['heartbeat']) ? $options['heartbeat'] : 0;
        return new static($host, $port, $user, $password, $vhost, $insist, $login_method, $login_response, $locale, $read_timeout, $keepalive, $write_timeout, $heartbeat);
    }
